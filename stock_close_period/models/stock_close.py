...
class StockClosePeriod(models.Model):
    ...
    def unlink(self):
        if any(closing.state in ["confirm", "done"] for closing in self):
            raise UserError(_("You can only delete in state 'Draft' or 'Cancelled'."))
    ...
    def _get_product_lines(self):
        ...
        self.env.cr.execute(query_add_products, param_add_products)
    ...
    def _deactivate_moves(self):
        ...
        """
        param_move_inactive = (
            fields.Date.to_string(self.close_date),
            self.company_id.id,
        )
        self.env.cr.execute(query_move_inactive, param_move_inactive)

        query_move_line_inactive = """
            UPDATE
                stock_move_line
            SET
                active = false
        ...
        """
        param_move_line_inactive = (
            fields.Date.to_string(self.close_date),
            self.company_id.id,
        )
        self.env.cr.execute(query_move_line_inactive, param_move_line_inactive)
        return True
        ...
        for closing in self:
            ...
            query_delete_zero_line = """
                ...
                WHERE
                    ...
                    AND price_unit = 0;
            """
            param_delete_zero_line = (closing.id,)
            self.env.cr.execute(query_delete_zero_line, param_delete_zero_line)
        return True
